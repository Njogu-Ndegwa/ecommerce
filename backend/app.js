...
app.post('/group_by_activityid', (req, res) => {
  ...
  if(view) {
    ...
    if(!time){
      ...
      RealPostgress.ReadQuery(`
      SELECT c.activity_id, DATE_TRUNC('${time}', c.ts) as ${time} from ${view} as c group by c.activity_id, DATE_TRUNC('${time}', c.ts) ORDER BY DATE_TRUNC('${time}', c.ts) ASC`, (data_set) =>{
        res.setHeader('Content-Type', 'application/json');
        const data = customParseInt(data_set)
        res.send(data.rows);
      })
...
app.post('/group_by_customer', (req, res) => {
  ...
  if(view) {
    ...
    if(!time){
      ...
      RealPostgress.ReadQuery(`
      SELECT c.customer, DATE_TRUNC('${time}', c.ts) as ${time} from generate_data_view as c group by c.customer, DATE_TRUNC('${time}', c.ts) ORDER BY DATE_TRUNC('${time}', c.ts) ASC`, (data_set) =>{
        res.setHeader('Content-Type', 'application/json');
        const data = customParseInt(data_set)
        res.send(data.rows);
      })
...
app.post('/group_by_timestamp', (req, res) => {
  ...
  if(view) {
    ...
    if(!time){
      ...
      RealPostgress.ReadQuery(`
      SELECT c.ts, DATE_TRUNC('${time}', c.ts) as ${time} from ${view} as c group by c.ts, DATE_TRUNC('${time}', c.ts) ORDER BY DATE_TRUNC('${time}', c.ts) ASC`, (data_set) =>{
        res.setHeader('Content-Type', 'application/json');
        const data = customParseInt(data_set)
        res.send(data.rows);
      })
...
app.post('/group_by_sourceid', (req, res) => {
  ...
  if(view) {
    ...
    if(!time){
      ...
      RealPostgress.ReadQuery(`
      SELECT c.source_id, DATE_TRUNC('${time}', c.ts) as ${time} from ${view} as c group by c.source_id, DATE_TRUNC('${time}', c.ts) ORDER BY DATE_TRUNC('${time}', c.ts) ASC`, (data_set) =>{
        res.setHeader('Content-Type', 'application/json');
          const data = customParseInt(data_set)
        res.send(data.rows);
      })
...
app.post('/group_by_activity', (req, res) => {
  ...
  if(view) {
    ...
    if(!time){
      ...
      RealPostgress.ReadQuery(`
      SELECT c.activity, DATE_TRUNC('${time}', c.ts) as ${time} from ${view} as c group by c.activity, DATE_TRUNC('${time}', c.ts) ORDER BY DATE_TRUNC('${time}', c.ts) ASC`, (data_set) =>{
        res.setHeader('Content-Type', 'application/json');
          const data = customParseInt(data_set)
        res.send(data.rows);
      })
...
app.post('/group_by_feature1', (req, res) => {
  ...
  if(view) {
    ...
    if(!time){
      ...
      RealPostgress.ReadQuery(`
      SELECT c.feature_1, DATE_TRUNC('${time}', c.ts) as ${time} from ${view} as c group by c.feature_1, DATE_TRUNC('${time}', c.ts) ORDER BY DATE_TRUNC('${time}', c.ts) ASC`, (data_set) =>{
        res.setHeader('Content-Type', 'application/json');
          const data = customParseInt(data_set)
        res.send(data.rows);
      })
...
app.post('/group_by_feature2', (req, res) => {
  ...
  if(view) {
    ...
    if(!time){
      ...
      RealPostgress.ReadQuery(`
      SELECT c.feature_2, DATE_TRUNC('${time}', c.ts) as ${time} from ${view} as c group by c.feature_2, DATE_TRUNC('${time}', c.ts) ORDER BY DATE_TRUNC('${time}', c.ts) ASC`, (data_set) =>{
        res.setHeader('Content-Type', 'application/json');
          const data = customParseInt(data_set)
        res.send(data.rows);
      })
...
app.post('/group_by_feature3', (req, res) => {
  ...
  if(view) {
    ...
    if(!time){
      ...
      RealPostgress.ReadQuery(`
      SELECT c.feature_3, DATE_TRUNC('${time}', c.ts) as ${time} from ${view} as c group by c.feature_3, DATE_TRUNC('${time}', c.ts) ORDER BY DATE_TRUNC('${time}', c.ts) ASC`, (data_set) =>{
        res.setHeader('Content-Type', 'application/json');
          const data = customParseInt(data_set)
        res.send(data.rows);
      })
...
app.post('/group_by_revenue_impact', (req, res) => {
  ...
  if(view) {
    ...
    if(!time){
      ...
      RealPostgress.ReadQuery(`
      SELECT c.revenue_impact, DATE_TRUNC('${time}', c.ts) as ${time} from ${view} as c group by c.revenue_impact, DATE_TRUNC('${time}', c.ts) ORDER BY DATE_TRUNC('${time}', c.ts) ASC`, (data_set) =>{
        res.setHeader('Content-Type', 'application/json');
          const data = customParseInt(data_set)
        res.send(data.rows);
      })
...
app.post('/group_by_link', (req, res) => {
  ...
  if(view) {
    ...
    if(!time){
      ...
      RealPostgress.ReadQuery(`
      SELECT c.link, DATE_TRUNC('${time}', c.ts) as ${time} from ${view} as c group by c.link, DATE_TRUNC('${time}', c.ts) ORDER BY DATE_TRUNC('${time}', c.ts) ASC`, (data_set) =>{
        res.setHeader('Content-Type', 'application/json');
          const data = customParseInt(data_set)
        res.send(data.rows);
      })
